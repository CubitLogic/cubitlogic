...
const EMOTION_PALETTES: Record<QubitEmotion, Palette> = {
  calm: {
    ...
    secondary: "#00E5FF",
    accent: "#DDFBFF",
    glow: "rgba(0, 153, 204, 0.48)",
    soft: "rgba(0, 153, 204, 0.10)",
  },
  curious: {
    label: "Curious",
    primary: "#6B21FF",
    secondary: "#A56BFF",
    accent: "#F0E8FF",
    glow: "rgba(107, 33, 255, 0.48)",
    soft: "rgba(107, 33, 255, 0.10)",
  },
  encouraging: {
    label: "Encouraging",
    primary: "#0E9F8A",
    secondary: "#35D6BC",
    accent: "#F2C94C",
    glow: "rgba(14, 159, 138, 0.48)",
    soft: "rgba(14, 159, 138, 0.10)",
  },
  confident: {
    label: "Confident",
    primary: "#1769E0",
    secondary: "#67B7FF",
    accent: "#FFFFFF",
    glow: "rgba(23, 105, 224, 0.50)",
    soft: "rgba(23, 105, 224, 0.10)",
  },
  excited: {
    label: "Excited",
    primary: "#00B8D9",
    secondary: "#7B2FFF",
    accent: "#F0E8FF",
    glow: "rgba(79, 74, 255, 0.52)",
    soft: "rgba(79, 74, 255, 0.10)",
  },
  concerned: {
    label: "Concerned",
    primary: "#D97706",
  ...
  reflective: {
    ...
    secondary: "#4E7AC7",
    accent: "#DCE9FF",
    glow: "rgba(33, 74, 138, 0.48)",
    soft: "rgba(33, 74, 138, 0.10)",
  },
  urgent: {
    label: "Urgent",
    primary: "#C62828",
...
export default function QubitVoiceOrb({
  ...
  const bars = useMemo(
    () => Array.from({ length: compact ? 9 : 13 }, (_, index) => index),
    [compact]
  );
    ...
    >
      <div className="relative flex-shrink-0" style={{ width: orbSize, height: orbSize }} aria-hidden="true">
        <motion.div
          ...
          animate={{
            scale: active ? [1, 1.13, 1] : [1, 1.045, 1],
            opacity: active ? [0.42, 0.75, 0.42] : [0.26, 0.42, 0.26],
          }}
          transition={{
        ...
        <motion.div
          ...
          style={{
            ...
            background: `radial-gradient(circle at 35% 28%, #ffffff 0%, ${palette.accent} 12%, ${palette.secondary} 38%, ${palette.primary} 70%, #050A1A 100%)`,
            boxShadow: `inset 0 0 22px rgba(255,255,255,0.55), 0 0 20px ${palette.glow}`,
          }}
        >
          <div className="absolute inset-0 opacity-30" style={{ background: "radial-gradient(circle at 70% 72%, transparent 0 35%, rgba(255,255,255,0.65) 37%, transparent 40%)" }} />
        </motion.div>
          ...
          {bars.map((index) => {
            ...
            return (
              <motion.span
                ...
                animate={{
                  height: active ? [idleHeight, activeHeight, idleHeight + 3] : idleHeight,
                  opacity: active ? [0.72, 1, 0.78] : 0.72,
                }}
                transition={{
                  duration: active ? 0.34 + (index % 4) * 0.05 : 0.45,
                ...
                style={{
                  width: compact ? 2 : 3,
                  maxHeight: compact ? 35 : 48,
                  background: index % 3 === 0 ? palette.accent : "rgba(255,255,255,0.94)",
      ...
      <div className={`min-w-0 flex-1 ${compact ? "" : "text-center sm:text-left"}`}>
        <div className="flex items-center gap-2 mb-1 justify-center sm:justify-start">
          ...
          <span
            ...
            style={{ color: palette.primary, backgroundColor: palette.soft, fontFamily: "'Space Grotesk', sans-serif" }}
          >
            {palette.label}
          </span>
        </div>
        <p className="text-xs text-gray-500 mb-3" style={{ fontFamily: "'Space Grotesk', sans-serif" }} aria-live="polite">
          {stateLabel}
        </p>
        ...
        <div className="flex items-center gap-2 justify-center sm:justify-start">
          <button
            ...
            aria-label={voiceState === "speaking" ? "Pause Qubit narration" : voiceState === "paused" ? "Resume Qubit narration" : "Play Qubit narration"}
          >
            {voiceState === "speaking" ? <Pause size={14} /> : <Play size={14} />}
            {voiceState === "speaking" ? "Pause" : voiceState === "paused" ? "Resume" : "Read aloud"}
          </button>
          <button
            type="button"
            onClick={stop}
